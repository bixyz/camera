...
class BluetoothHIDController:
    ...
    def cleanup_existing_profile(self):
        """清理已存在的HID Profile"""
        try:
            ...
            logger.info("已清理现有的HID Profile")
            time.sleep(1)  # 等待清理完成
            
        except Exception as e:
            logger.info(f"清理现有Profile时出错（可能本来就没有）: {e}")
    ...
    def setup_bluetooth(self):
        ...
        try:
            ...
            # 创建HID服务
            self.hid_service = BluetoothHIDService(self.bus, '/org/bluez/hid')
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info("蓝牙HID服务已注册")
                    break
                    
                except dbus.exceptions.DBusException as e:
                    if 'UUID already registered' in str(e) and attempt < max_retries - 1:
                        logger.warning(f"UUID已注册，尝试清理并重试 (第{attempt + 1}次)")
                        self.cleanup_existing_profile()
                        time.sleep(2)
                    else:
                        raise e
            
            return self.profile_registered
...
<record>
    ...
    <attribute id="0x0100">
        <text value="RaspberryPi HID" />
    </attribute>
    <attribute id="0x0101">
        <text value="Virtual Touch Pad" />
    </attribute>
    <attribute id="0x0102">
        <text value="RaspberryPi" />
    </attribute>
    <attribute id="0x0200">
        <uint16 value="0x0100" />
    </attribute>
    ...
    def run(self):
        ...
        print("蓝牙HID服务已启动，等待iPhone连接...")
        print("在iPhone的蓝牙设置中搜索并连接到 'RaspberryPi HID'")
